...
:::steps
- 3.1 ta-assistant 本体 | 命令行直接调 bridge,**不经 OpenClaw** —— 这段通了说明后端没毛病,问题在后面几跳
- 3.2 大模型通道 | 额度 / 权限 / 网络;坏了**不会崩**,只降级成"资料原文"
- 3.3 OpenClaw 网关与微信渠道 | Bot 到底能不能收发
- 3.4 agent 路由 | 直接给 agent 发消息(**不经过微信**),看它调不调后端
- 3.5 微信真机 | 最后才在微信里试 —— 前面四段都绿了再进来
:::

:::note 为什么必须从小到大
链路上有四个能"安静地失败"的地方:插件没连上、agent 自己瞎答、
后端降级成资料原文、模型回退到另一个模型。**每一跳的表现都差不多**
——微信那头都是"回复得不太对"。从最小的一段开始查,每次只引入一个新变量,
才能知道是哪一跳坏了;一上来就在微信里试,等于同时怀疑四个人。
:::
...
:::timeline
- 2026-09-17 | 首次实测 | 微信端到端第一次跑通,一次撞出 7.1–7.5 五条
- 2026-09-20 | 额度用尽 | MiniMax 探活返回 `2056`,agent 和本项目两边都切 OpenCode Go ✅
- 2026-09-20 | 资料目录名 | 7.6 结案 ✅
- 2026-09-20 晚 | 概念题 | 不再一律回"建议问任课老师",改成按三档规则讲 ✅
- 2026-09-20 深夜 | 语料 | 作业答案统一取**中文版**参数 ✅;中文版第 6 版教材 OCR 进库 ✅
- 2026-09-20 更晚 | 检索闸门 | 修 BM25 的 IDF 兜底 + 建 17 条常驻评测 ✅
- 仍未解决 | 7.3 补交去重 / 7.4 自传记忆 / 7.5 Fallback 字样 | ⚠️ 详见下面各节
:::
