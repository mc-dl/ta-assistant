...
:::cards
- US-1 自动登记补交 | 转发学生的补交消息 → 系统自己认出姓名/学号/第几次 → 写进 Excel
- US-2 自动答常见问题 | "Proteus 在哪下载"这类重复问题,直接按 FAQ 答,助教只审一眼
- US-3 自动答知识问题 | "什么是竞争冒险" → 从资料里找相关片段,让大模型据此作答
- US-4 所有文件在一处 | 代码 / 文档 / 日志 / 索引都别散落
:::
...
```mermaid
flowchart TD
  %% caption: 功能需求总览 —— F-1 收口,分类器分三路,写通路落两个文件
  F6["<b>F-6</b> OpenClaw 接入<br/>子进程 + 一行 JSON"]
  F1["<b>F-1</b>【准入】消息前缀过滤<br/>没有 【转发学生提问】 前缀 → skip"]
  F2["<b>F-2</b>【路由】消息分类器<br/>规则优先 + LLM 兜底"]
  F3["<b>F-3</b>【写通路】补交登记<br/>抽学号/姓名/次数 → 写 Excel"]
  F7["<b>F-7</b>【写通路】助教写 FAQ<br/>四道检查 → 原子追加"]
  F4["<b>F-4</b>【答通路】FAQ 检索<br/>阈值 4.0 + 实词闸门"]
  F5["<b>F-5</b>【答通路】课程知识 RAG<br/>BM25 top-7 + 三档口径"]
  XLS["补交表.xlsx"]
  TXT["常问问题.txt"]
  LLM["大模型润色"]
...
```mermaid
flowchart LR
  %% caption: main.process() 的两道闸门 —— 【记录】必须在学生转发闸门之前
  MSG["学生转发来的消息"] --> G1{"①【记录】闸门<br/>以 【记录】 开头?"}
  G1 -->|"是"| W["写 FAQ<br/>不经过分类器"]
  G1 -->|"不是"| G2{"②学生转发闸门<br/>以 【转发学生提问】 开头?"}
  G2 -->|"是"| C["进分类器<br/>submission / question"]
  G2 -->|"不是"| S["skip<br/>(非学生消息,未处理)"]
```

> 两道闸门**调换顺序就会静默出错**:`【记录】…` 的消息没有 `【转发学生提问】` 前缀,
> 先过②就会被判成"非学生消息"丢掉 —— 而日志里只会多一条正常的 skip,
> 看上去一切正常。`test/` 里有一条用例专门钉这个顺序。
...
- **数据源**(**【当前】都在数据目录 `WINDOWS_ROOT` 下,默认 `~/ykt_questions/`**,
  ...
  - 补交记录表:**由花名册路径推导** —— 同目录(`花名册/`)、同课名、后缀换成「补交表」;
    本学期的名单是 `电路基础理论课_学生名单.xlsx`,所以表是 `花名册/电路基础理论课_补交表.xlsx`
    (读写;**不存在则自动创建**,字段见下;但**所在目录不存在时不建** —— 那说明路径配错了,
    见 `knowledge_base.md` §9 里那次"补交静默走进空表"的事故)
- **补交记录表字段**(如不存在自动创建):
...
- ❌ **不做题图识别** —— 学生发的截图只取文字,题图到现在还是最大的洞

---

### 说不做,**后来破例做了**(附原因)
- ⚠️ "不做 OCR" —— 学生截图仍然不做;但**扫描版教材**没有文字层,不 OCR 就进不了库,所以单独开了脚本 + 独立 venv(见 `knowledge_base.md` §3.5)
- ⚠️ "不做多课程支持" —— 现在服务**两门课**(数电实验 + 电路基础),原因和代价见本文开头的对照表
:::
